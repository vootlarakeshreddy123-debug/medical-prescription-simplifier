...
async function startServer() {
  ...
  // Diagnostic middleware to verify that Vercel/Express
  // is actually matching the prescription upload route.
  app.use(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (!text && !imageBase64) {
          const errorBody = {
            ...
            error: {
              code: 'INVALID_IMAGE',
              message:
                'Please provide prescription text, an image, or a document file to analyze.',
            },
          };
        ...
        console.log('Vision result:', {
          provider:
            extracted?.visionOcrResult?.provider ||
            (cleanImageBase64
              ? 'google_cloud_vision'
              : 'text_parser'),
          detectedLines:
            extracted?.visionOcrResult?.lines?.length || 0,
          ocrConfidence:
            extracted?.visionOcrResult?.confidence || 85,
        });
        ...
        console.log('Gemini result:', {
          ...
          overallConfidence:
            extracted?.overallConfidence || 'medium',
          confidenceScore:
            extracted?.confidenceScore || 80,
        });
        ...
        // Build simplified schedule groups from extracted medicines
        // according to doctor's explicit instructions.
        ...
        for (const med of extracted.medicines) {
          ...
          const timing = Array.isArray(med.timingOfDay)
            ? med.timingOfDay
            : [];
        ...
        const prescription: Prescription = {
          ...
          sourceType:
            sourceType ||
            (cleanImageBase64 ? 'image' : 'text'),
          rawInputText: text,
          imageUrl: cleanImageBase64
            ? `data:${cleanMimeType};base64,${cleanImageBase64}`
            : undefined,
          overallConfidence:
          ...
          safetyFindings:
            extracted.safetyFindings,
          interactions:
            extracted.interactions,
          duplicateDetections:
            extracted.duplicateDetections,
          questionsForDoctor:
            extracted.questionsForDoctor,
          abbreviationsFound:
            extracted.abbreviationsFound,
          simplifiedSummary:
            extracted.simplifiedSummary,
          language: lang,
        ...
        db.addChatMessage(saved.id, {
          ...
          timestamp: new Date().toISOString(),
          suggestedQuestions:
            saved.questionsForDoctor.slice(0, 3),
        });
        ...
        const responsePayload = {
          ...
          data: {
            ...saved,
            doctor_instructions:
              saved.simplifiedSummary ||
              (saved.doctorName
                ? `Prescribed by Dr. ${saved.doctorName}`
                : ''),
            warnings: (
              saved.safetyFindings || []
            ).map(
        ...
        console.log('Backend final response:', {
          success: true,
          medicineCount:
            responsePayload.data.medicines.length,
          prescriptionId: saved.id,
          confidenceScore:
            responsePayload.data.confidenceScore,
          schedule: {
            morning:
              responsePayload.data
                .simplifiedSchedule.morning.length,
            afternoon:
              responsePayload.data
                .simplifiedSchedule.afternoon.length,
            evening:
              responsePayload.data
                .simplifiedSchedule.evening.length,
            bedtime:
              responsePayload.data
                .simplifiedSchedule.bedtime.length,
            asNeeded:
              responsePayload.data
                .simplifiedSchedule.asNeeded.length,
            unclear:
              responsePayload.data
                .simplifiedSchedule.unclear.length,
          },
        });
        ...
        if (
          ...
          const errorBody = {
            ...
            error: {
              ...
              message:
                err?.message ||
                'Failed to process document image with Google Cloud Vision OCR.',
            },
          };
        ...
        if (
          ...
          const errorBody = {
            ...
            error: {
              ...
              message:
                err?.message ||
                'No legible text or medical prescriptions were detected in this image. Please upload a clearer photo or enter details manually.',
            },
          };
        ...
        if (
          ...
          const errorBody = {
            ...
            error: {
              ...
              message:
                err?.message ||
                'Invalid image format or corrupted file provided. Please provide a clear JPEG, PNG, or WebP image.',
            },
          };
        ...
        if (
          ...
          const errorBody = {
            ...
            error: {
              ...
              message:
                err?.message ||
                'AI model generated an incomplete or invalid response format.',
            },
          };
        ...
        const errorBody = {
          ...
          error: {
            ...
            message:
              err?.message ||
              'An unexpected internal error occurred while analyzing the prescription.',
          },
        };
    ...
    (req: Request, res: Response) => {
      try {
        ...
        const formattedMeds: Medicine[] =
          medicines.map(
            (m: any, idx: number) => ({
              ...
              instructions:
                m.instructions ||
                'Take as instructed by doctor.',
              timingOfDay:
                m.timingOfDay || ['morning'],
              withFood:
                m.withFood || 'unspecified',
              confidence: {
                name: 'high' as const,
        ...
        for (const med of formattedMeds) {
          ...
          if (
            med.timingOfDay.includes('morning')
          ) {
            morningList.push({
              ...item,
              timingCategory: 'morning',
          ...
          if (
            med.timingOfDay.includes('afternoon')
          ) {
            afternoonList.push({
              ...item,
              timingCategory: 'afternoon',
          ...
          if (
            med.timingOfDay.includes('evening')
          ) {
            eveningList.push({
              ...item,
              timingCategory: 'evening',
          ...
          if (
            med.timingOfDay.includes('bedtime')
          ) {
            bedtimeList.push({
              ...item,
              timingCategory: 'bedtime',
          ...
          if (
            med.timingOfDay.includes('as_needed')
          ) {
            asNeededList.push({
              ...item,
              timingCategory: 'as_needed',
          ...
          if (
            med.timingOfDay.includes('unclear')
          ) {
            unclearList.push({
              ...item,
              timingCategory: 'unclear',
        ...
        console.error(
          'Manual prescription entry error:',
          err
        );

        res.status(500).json({
          error:
            'Failed to save manual prescription',
        });
      }
    }
  );
  ...
  app.post(
    '/api/prescriptions/demo',
    (req: Request, res: Response) => {
      const userId = getUserIdFromReq(req);
      const demo =
        db.resetDemoData(userId);
  ...
  app.delete(
    ...
    (req: Request, res: Response) => {
      ...
      res.json({
        message:
          'Prescription permanently deleted',
      });
    }
  );
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const aiResponse =
          await aiService.answerPrescriptionQuestion(
            rx,
            history,
            message,
        ...
        if (
          ...
          return res.status(503).json({
            error:
              err.message ||
              'Gemini API is not configured. Add the required API key to your .env file.',
          });
        }
  ...
  app.post(
    '/api/prescriptions/:id/translate',
    async (req: Request, res: Response) => {
      try {
        const userId =
          getUserIdFromReq(req);
        ...
        const translation =
          await aiService.translatePrescription(
            rx,
            lang
          );
        ...
        res.status(500).json({
          error:
            'Failed to translate prescription',
        });
      }
    }
  );
    ...
    (req: Request, res: Response) => {
      ...
      if (
        ...
        return res.json({
          interactions: [],
          message:
            'Provide at least two medicine names to perform interaction checks.',
        });
      }

      const medNames =
        medicines.map((m: string) =>
          m.toLowerCase()
        );
  ...
  app.get(
    ...
    (req: Request, res: Response) => {
      ...
      res.json({
        found: false,
        message:
          'Medicine not in predefined offline library. Consult your healthcare provider.',
      });
    }
  );
  ...
  app.get(
    '/api/internal/ocr-benchmark',
    async (req: Request, res: Response) => {
      try {
        const benchmarkReport =
          await BenchmarkEvaluator.evaluateSuite();
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        const cases =
          await TestingService.getTestCases();

        res.json(cases);
      } catch (err: any) {
        res.status(500).json({
          error:
            'Failed to retrieve testing cases',
          details: err?.message,
        });
      }
    }
  ...
  app.get(
    '/api/testing/cases/:id',
    async (req: Request, res: Response) => {
      try {
        const testCase =
          await TestingService.getTestCase(
            req.params.id
          );
  ...
  app.get(
    '/api/testing/cases/:id/image',
    async (req: Request, res: Response) => {
      try {
        const img =
          await TestingService.getImageBuffer(
            req.params.id
          );
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        const metrics =
          await TestingService.getDashboardMetrics();

        res.json(metrics);
      } catch (err: any) {
        res.status(500).json({
          error:
            'Failed to calculate testing metrics',
          details: err?.message,
        });
      }
    }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (
          ...
          return res.status(400).json({
            error:
              'Payload must contain an "items" array with test image items',
          });
        }

        const created =
          await TestingService.addTestCases(
            items
          );
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        const updated =
          await TestingService.updateTestCase(
            req.params.id,
            req.body
          );
  ...
  app.post(
    '/api/testing/batch-run',
    async (req: Request, res: Response) => {
      try {
        const { ids } = req.body;

        const results =
          await TestingService.runBatch(
            ids
          );
        ...
        res.status(500).json({
          error:
            'Failed to execute batch test run',
          details: err?.message,
        });
      }
    }
  ...
  // Automated Batch Queue: Run all unevaluated tests
  // with safe concurrency & live progress
  app.post(
    '/api/testing/batch/run-unevaluated',
    async (req: Request, res: Response) => {
      try {
        const status =
          await BatchQueueService.startOrResumeBatch();
        ...
        res.status(500).json({
          error:
            'Failed to start unevaluated batch job',
          details: err?.message,
        });
      }
    }
  ...
  app.get(
    '/api/testing/batch/status',
    async (req: Request, res: Response) => {
      try {
        const status =
          await BatchQueueService.getStatus();
  ...
  app.post(
    '/api/testing/batch/pause',
    (req: Request, res: Response) => {
      try {
        const status =
          BatchQueueService.pauseBatch();
  ...
  app.post(
    '/api/testing/batch/resume',
    (req: Request, res: Response) => {
      try {
        const status =
          BatchQueueService.resumeBatch();
  ...
  app.post(
    '/api/testing/batch/stop',
    (req: Request, res: Response) => {
      try {
        const status =
          BatchQueueService.stopBatch();
  ...
  app.post(
    '/api/testing/recalculate',
    async (req: Request, res: Response) => {
      try {
        const result =
          await TestingService.recalculateAllTestCases();
        ...
        res.status(500).json({
          error:
            'Failed to recalculate test cases',
          details: err?.message,
        });
      }
    }
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res.status(500).json({
          error:
            'Failed to re-seed demo test cases',
          details: err?.message,
        });
      }
    }
  ...
  app.delete(
    ...
    async (req: Request, res: Response) => {
      try {
        const deleted =
          await TestingService.deleteTestCase(
            req.params.id
          );
  ...
  app.get(
    '/api/testing/export-csv',
    async (req: Request, res: Response) => {
      try {
        const csv =
          await TestingService.exportCSV();
        ...
        res.setHeader(
          'Content-Disposition',
          'attachment; filename="prescription_evaluation_results.csv"'
        );
  ...
  app.get(
    '/api/testing/feedback-dataset',
    (req: Request, res: Response) => {
      try {
        const records =
          FeedbackDatasetService.getAllRecords();

        const stats =
          FeedbackDatasetService.getStats();
        ...
        res.status(500).json({
          error:
            'Failed to load feedback dataset',
          details: err?.message,
        });
      }
    }
  ...
  app.get(
    '/api/testing/feedback-dataset/stats',
    (req: Request, res: Response) => {
      try {
        const stats =
          FeedbackDatasetService.getStats();
  ...
  app.post(
    ...
    (req: Request, res: Response) => {
      try {
        ...
        res.status(500).json({
          error:
            'Failed to record feedback item',
          details: err?.message,
        });
      }
    }
  ...
  app.get(
    '/api/testing/feedback-dataset/export',
    (req: Request, res: Response) => {
      try {
        const format =
        ...
        if (format === 'csv') {
          const csv =
            FeedbackDatasetService.exportCSV();
          ...
          res.setHeader(
            'Content-Disposition',
            'attachment; filename="feedback_learning_dataset.csv"'
          );
        ...
        const json =
          FeedbackDatasetService.exportJSON();
        ...
        res.setHeader(
          'Content-Disposition',
          'attachment; filename="feedback_learning_dataset.json"'
        );
        ...
        res.status(500).json({
          error:
            'Failed to export feedback dataset',
          details: err?.message,
        });
      }
    }
  ...
  // ----------------------------------------------------
  // API 404 HANDLER
  // Ensures /api/* always returns JSON,
  // never HTML.
  // ----------------------------------------------------
...
if (!process.env.VERCEL) {
  appReady
    ...
    .catch((error) => {
      console.error(
        'Failed to start Medical Prescription Simplifier server:',
        error
      );
